...
import { saveInvoice, bumpCounter, getNextInvoiceNo } from '../utils/storage'
import { generateInvoicePDF } from '../utils/pdf'

const today  = () => dayjs().format('DD/MM/YYYY')
const blankClient = () => ({ name: '', address: '', stateCode: '06', gstin: '' })
const blankItem   = () => ({
  id: Date.now(),
...
function Input({ value, onChange, placeholder = '', type = 'text', className = '' }) {
  return (
    <input
      ...
      className={`w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white ${className}`}
    />
  )
}
  ...
  return (
    <>
      <Field label="Company Name">
        <Input value={value.name} onChange={v => set('name', v)} placeholder="e.g. Bellsonica Auto Component India Pvt. Ltd." />
      </Field>
      <Field label="Address">
        <textarea
          value={value.address}
          ...
          rows={2}
          placeholder="Plot No-1, Phase 3A, IMT Manesar, Gurgaon (122051)"
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="State Code">
          <Input value={value.stateCode} onChange={v => set('stateCode', v)} placeholder="06" />
        </Field>
        <Field label="GSTIN">
          <Input value={value.gstin} onChange={v => set('gstin', v.toUpperCase())} placeholder="06XXXXXX" />
        </Field>
      </div>
    </>
  )
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-3 flex flex-col gap-2.5 bg-gray-50">
      ...
      <Field label="Description">
        <Input value={item.description} onChange={v => set('description', v)} placeholder="Hydra F-17 Charges for..." />
      </Field>

      <div className="grid grid-cols-2 gap-2">
        <Field label="Item Code (optional)">
          <Input value={item.itemCode} onChange={v => set('itemCode', v)} placeholder="" />
        </Field>
        <Field label="HSN / SAC">
          <Input value={item.hsnSac} onChange={v => set('hsnSac', v)} placeholder={DEFAULTS.hsnSac} />
        </Field>
      </div>

      <div className="grid grid-cols-3 gap-2">
        <Field label="Quantity">
          <Input value={item.quantity} onChange={v => set('quantity', v)} type="number" placeholder="30.5" />
        </Field>
        <Field label="Unit">
          <select
            value={item.unit}
            onChange={e => set('unit', e.target.value)}
            className="w-full border border-gray-300 rounded-lg px-2 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
          >
            {DEFAULTS.units.map(u => <option key={u}>{u}</option>)}
          </select>
        </Field>
        <Field label="Rate (₹)">
          <Input value={item.rate} onChange={v => set('rate', v)} type="number" placeholder="1875" />
        </Field>
      </div>
...
export default function InvoiceForm({ initial, onSave }) {
  ...
  function updateItem(id, updated) {
    set('items', inv.items.map(it => it.id === id ? updated : it))
  }
  function addItem() {
    set('items', [...inv.items, blankItem()])
  }
  function removeItem(id) {
    set('items', inv.items.filter(it => it.id !== id))
  }
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <Section title="Invoice Details">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Invoice Date">
            <Input value={inv.invoiceDate} onChange={v => set('invoiceDate', v)} placeholder="DD/MM/YYYY" />
          </Field>
        </div>
        <Field label="Place of Supply">
          <Input value={inv.placeOfSupply} onChange={v => set('placeOfSupply', v.toUpperCase())} placeholder="GURGAON" />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="PO No.">
      ...
      <Section title="Billed To">
        <ClientFields value={inv.billedTo} onChange={v => set('billedTo', v)} />
      </Section>
      ...
      <Section title="Shipped To">
        <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
          <input
            type="checkbox"
            checked={inv.shippedSameAsBilled}
        ...
        {!inv.shippedSameAsBilled && (
          <ClientFields value={inv.shippedTo} onChange={v => set('shippedTo', v)} />
        )}
      </Section>
      ...
      <Section title="Payment & Tax">
        <Field label="Payment Terms">
          <Input value={inv.paymentTerms} onChange={v => set('paymentTerms', v)} placeholder="Within 30 Days" />
        </Field>
            ...
            {[['intra', 'CGST + SGST (Same State)'], ['inter', 'IGST (Different State)']].map(([val, label]) => (
              <label key={val} className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  name="gstType"
                  value={val}
                  checked={inv.gstType === val}
                  onChange={() => set('gstType', val)}
